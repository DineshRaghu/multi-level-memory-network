...
all_entities = json.load(open("../data/Maluuba/entities.json"))
outs = []
golds = []
...
for i in range(0,len(data['sentences'])):
	...
	gold_anon = [vocab['rev_mapping'][word].encode('ascii',errors='ignore') for word in ground_truth ]
	out_anon = [vocab['rev_mapping'][word].encode('ascii',errors='ignore') for word in predicted ]

	for word in out_anon:
		if word in entities or word.isdigit() or word in all_entities:
