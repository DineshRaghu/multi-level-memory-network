...
class DialogueModel(object):
	...
	def __init__(self,device,batch_size,inp_vocab_size,out_vocab_size,generate_size,emb_init,emb_dim,enc_hid_dim,dec_hid_dim,attn_size,dropout_keep_prob):
		...
		self.kb = tf.placeholder(
			name='kb', dtype=tf.int64,
			shape=[self.batch_size,None,6],
		)

		self.kb_mask = tf.placeholder(
			name='kb_mask', dtype=tf.float32,
			shape=[self.batch_size,None],
		)

		self.keys = tf.placeholder(
			name='keys', dtype=tf.int64,
			shape=[self.batch_size,None,6],
		)

		self.keys_mask = tf.placeholder(
			name='keys_mask', dtype=tf.float32,
			shape=[self.batch_size,None,6],
		)
	...
	def buildArch(self):
		...
		with tf.device(self.device):
			...
			self.kb_emb = embedding_ops.embedding_lookup(self.embeddings, self.kb)
			self.keys_emb = embedding_ops.embedding_lookup(self.embeddings, self.keys)
			self.result_rep = tf.einsum('ij,ijk->ijk',tf.pow(tf.reduce_sum(self.keys_mask,2),-1),tf.reduce_sum(tf.einsum('ijk,ijkl->ijkl',self.keys_mask,self.kb_emb),2))
	...
	def create_output_unit(self):
		...
		def unit(hidden_state):
			...
			hidden_state_expanded_result = tf.tile(array_ops.expand_dims(hidden_state,1),[1,tf.shape(self.kb)[1],1])
			inp_attn_expanded_result = tf.tile(array_ops.expand_dims(inp_attn,1),[1,tf.shape(self.kb)[1],1])
			result_attn_rep = tf.concat([self.result_rep,hidden_state_expanded_result,inp_attn_expanded_result],axis=2)
			result_attn_rep = tf.nn.tanh(tf.einsum("ijk,kl->ijl",tf.nn.tanh(tf.einsum("ijk,kl->ijl",result_attn_rep,self.W_1)),self.W_12))
			beta_logits = tf.squeeze(tf.einsum('ijk,kl->ijl',result_attn_rep,self.r_1),2)
			beta_masked = tf.multiply(tf.cast(self.kb_mask,dtype=tf.float64),tf.exp(tf.cast(beta_logits,dtype=tf.float64)))
			beta = tf.cast(tf.einsum('i,ij->ij',tf.pow(tf.reduce_sum(beta_masked,1),-1),beta_masked),dtype=tf.float32)
			
			hidden_state_expanded_keys = tf.tile(array_ops.expand_dims(array_ops.expand_dims(hidden_state,1),1),[1,tf.shape(self.kb)[1],tf.shape(self.kb)[2],1])
			inp_attn_expanded_keys = tf.tile(array_ops.expand_dims(array_ops.expand_dims(inp_attn,1),1),[1,tf.shape(self.kb)[1],tf.shape(self.kb)[2],1])
			result_key_rep = tf.concat([self.keys_emb,hidden_state_expanded_keys,inp_attn_expanded_keys],axis=3)
			result_key_rep = tf.nn.tanh(tf.einsum('ijkl,lm->ijkm',tf.nn.tanh(tf.einsum('ijkl,lm->ijkm',result_key_rep,self.W_2)),self.W_22))
			gamma_logits =  tf.squeeze(tf.einsum('ijkl,lm->ijkm',result_key_rep,self.r_2),3)
			gamma_masked = tf.multiply(tf.cast(self.keys_mask,dtype=tf.float64),tf.exp(tf.cast(gamma_logits,dtype=tf.float64)))
			gamma =  tf.einsum('ij,ijk->ijk',beta,tf.cast(tf.einsum('ij,ijk->ijk',tf.pow(tf.reduce_sum(gamma_masked,2),-1),gamma_masked),dtype=tf.float32))
			...
			db_rep = tf.reduce_sum(tf.einsum('ij,ijk->ijk',beta,self.result_rep),1)
	...
	def get_feed_dict(self,batch):
		...
		fd = {
			...
			self.kb : batch['kb'],
			self.kb_mask : batch['kb_mask'],
			self.keys : batch['keys'],
			self.keys_mask : batch['keys_mask'],
